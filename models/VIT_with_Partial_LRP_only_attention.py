...
def lrp_linear(a: torch.Tensor, W: torch.Tensor, R_out: torch.Tensor,
    ...
    if a.dim() == 2:
        ...
        # 修正數值穩定性：使用更穩定的 epsilon 規則
        denom = z + eps * torch.sign(z) + eps  # 額外加 eps 避免除零
        ...
        # 檢查並處理 NaN
        Rin = torch.where(torch.isnan(Rin), torch.zeros_like(Rin), Rin)
        return Rin
    ...
    def explain(self, x: torch.Tensor, target_class: Optional[int] = None,
        ...
        for blk in reversed(self.cache):
            ...
            if return_head_importance:
                # 計算當前層索引（從後往前，因為我們在 reversed 迴圈中）
                current_layer = len(self.cache) - 1
