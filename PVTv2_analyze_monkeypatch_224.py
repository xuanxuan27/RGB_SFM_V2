...
def get_stage_target_size(layer_name: str, input_size: int) -> int:
    """
    根據 layer_name 和 input_size 計算對應 stage 的目標解析度。
    
    PVTv2 的 stage 解析度變化：
    - Stage 1: input_size / 4  (stride=4)
    - Stage 2: input_size / 8  (stride=4*2)
    - Stage 3: input_size / 16 (stride=4*2*2)
    - Stage 4: input_size / 32 (stride=4*2*2*2)
    
    Args:
        layer_name: 例如 "stage1_block1", "stage2_block2" 等
        input_size: 輸入圖像大小（例如 224）
    
    Returns:
        該 stage 的目標解析度（例如 56, 28, 14, 7）
    """
    if layer_name.startswith("stage"):
        try:
            stage_num = int(layer_name.split("_")[0].replace("stage", ""))
            # Stage 1: /4, Stage 2: /8, Stage 3: /16, Stage 4: /32
            target_size = input_size // (4 * (2 ** (stage_num - 1)))
            return max(1, target_size)  # 確保至少為 1
        except (ValueError, IndexError):
            # 如果解析失敗，返回預設值（對應 Stage 4）
...
def save_heads_heatmap_montage(
    ...
    """
    Paper-like montage:
    - All heads share the SAME vmin/vmax
    - Only ONE shared colorbar
    - White background, compact titles
    - Attention maps are upsampled to target_size using bilinear interpolation
    """
    print(f"target_size: {target_size}")
    print(f"Hk: {Hk}, Wk: {Wk}")
    heads = key_imp.shape[0]
    cols = min(cols, heads)
    rows = int(math.ceil(heads / cols))

    all_vals = key_imp.numpy().reshape(-1)
    vmin = 0.0
    vmax = float(np.percentile(all_vals, vmax_percentile))
    if vmax <= 1e-8:
        vmax = float(all_vals.max() + 1e-8)

    fig = plt.figure(figsize=(3.6 * cols, 3.2 * rows), facecolor="white")
    gs = fig.add_gridspec(rows, cols + 1, width_ratios=[1] * cols + [0.05], wspace=0.15, hspace=0.25)
    ...
    for h in range(heads):
        ...
        # Upsample to target_size using bilinear interpolation
        hm_upsampled = cv2.resize(hm, (target_size, target_size), interpolation=cv2.INTER_LINEAR)
        last_im = ax.imshow(hm_upsampled, cmap=cmap, vmin=vmin, vmax=vmax, interpolation="bilinear")
        ax.set_title(f"H{h+1}", fontsize=11)
        ax.axis("off")
    ...
    fig.suptitle(layer_name, fontsize=14, fontweight="bold", y=0.98)
    fig.savefig(fig_path, dpi=220, facecolor="white", bbox_inches="tight")
    plt.close(fig)
...
def save_heads_overlay_montage(
    ...
    """Overlay montage: no per-head colorbar, clean layout."""
    heads = len(overlays_bgr)
    cols = min(cols, heads)
    rows = int(math.ceil(heads / cols))

    fig, axes = plt.subplots(rows, cols, figsize=(3.6 * cols, 3.2 * rows), facecolor="white")
    axes = np.array(axes).reshape(-1)

    for h in range(heads):
        rgb = cv2.cvtColor(overlays_bgr[h], cv2.COLOR_BGR2RGB)
        axes[h].imshow(rgb)
        axes[h].set_title(f"H{h+1}", fontsize=11)
        axes[h].axis("off")

    for j in range(heads, len(axes)):
        axes[j].axis("off")

    fig.suptitle(layer_name + " (masked overlay)", fontsize=14, fontweight="bold", y=0.98)
    plt.tight_layout()
    plt.savefig(fig_path, dpi=220, facecolor="white", bbox_inches="tight")
    plt.close(fig)
...
def save_heads_topk_montage(
    ...
    """Top-k montage: points + rank labels."""
    ...
    fig.suptitle(layer_name + f" (top-k points)", fontsize=14, fontweight="bold", y=0.98)
    plt.tight_layout()
    plt.savefig(fig_path, dpi=220, facecolor="white", bbox_inches="tight")
    plt.close(fig)
...
def analyze_and_visualize(
    ...
    for layer_name in layer_keys:
        ...
        # Calculate target size based on stage resolution
        stage_target_size = get_stage_target_size(layer_name, input_size)
        ...
        save_heads_heatmap_montage(
            ...
            target_size=stage_target_size,  # Upsample to stage-specific resolution using bilinear interpolation
        )
...
def build_argparser():
    ...
    ap.add_argument("--save_dir", type=str, default="pvt_v2_attn_vis_clean_224", help="output directory")
